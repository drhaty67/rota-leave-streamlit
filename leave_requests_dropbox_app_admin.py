...
# Load requests and consultant list
df = load_requests()
...
st.write(
    ...
    "Restrict this to the rota administrator to avoid Dropbox Excel conflicts."
)
...
try:
    admin_password_secret = st.secrets.get("ROTA_ADMIN_PASSWORD", "")
except Exception:
    admin_password_secret = ""
...
if not admin_password_secret:
    st.warning("Set `ROTA_ADMIN_PASSWORD` in `.streamlit/secrets.toml` to restrict compile.")
else:
    if not st.session_state["admin_authed"]:
        with st.form("admin_login"):
            pw = st.text_input("Admin password", type="password")
            login = st.form_submit_button("Unlock compile")
        if login:
            if pw == admin_password_secret:
        ...
        st.info("Admin access granted. Compile controls are enabled for this session.")
        if st.button("Lock compile"):
            st.session_state["admin_authed"] = False
            st.rerun()
...
if not workbook_path_str:
    ...
    compile_enabled = (not admin_password_secret) or st.session_state.get("admin_authed", False)

    if not compile_enabled:
        st.warning("Compile is locked. Enter the admin password above to enable it.")
    else:
